...
from notebooks.validate_after_preproc import validate_after_preprocessing
...
def preprocess_data(df):
    ...
    model_Amenity = Word2Vec(sentences=df['clean_description_col_processed'], vector_size=10, window=5, min_count=1, workers=4)
    # Getting vectors for words
    word_vectors_Amenity = model_Amenity.wv
    # Getting the average vector for each document
    df['clean_description_col_vector'] = df['clean_description_col_processed'].apply(lambda x: sum([word_vectors_Amenity[word] for word in x if word in word_vectors_Amenity] or [0]) / len(x))
    ...
    df['clean_string_col_vector'] = df['clean_string_col_processed'].apply(lambda x: sum([word_vectors[word] for word in x if word in word_vectors] or [0]) / len(x))
    ...
    word_vectors = model.wv
    # Getting the average vector for each document
    df['clean_rn_col_vector'] = df['clean_rn_col_processed'].apply(lambda x: sum([word_vectors[word] for word in x if word in word_vectors] or [0]) / len(x))


    df.drop([clean_string_col, clean_description_col, 'clean_description_col_processed', 'clean_string_col_processed', clean_rn_col, 'clean_rn_col_processed'], axis=1, inplace=True)
