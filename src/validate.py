...
def run_giskard_tests(giskard_model, giskard_dataset, model_name, model_version,
    ...
    test_suite.add_test(rmse_test_fn, model=giskard_model, data=giskard_dataset,
                        display_name=f"MAE Test (Threshold {mae_threshold})", test_id="mae_test")
...
def validate_model(model_name, model_alias, model_version,
                   data_version, mae_threshold, giskard_dataset, cfg):
    ...
    test_results = run_giskard_tests(giskard_model, giskard_dataset, model_name, model_version,
                                     data_version, mae_threshold)
    return scan, test_results, model_version


def validate_all_models(model_name, data_version, mae_threshold, giskard_dataset, cfg):
    client = mlflow.MlflowClient()
...
def main():
    with initialize(config_path="../configs", job_name="validate", version_base=None):
        ...
        parser.add_argument("--model-name", type=str, default="gbr", help="Model name (gbr or dtr)",
                            required=True)
        ...
        parser.add_argument("--mae-threshold", type=float, default=10, help="MAE threshold for tests")
        ...
        if args.validate_all:
            ...
            best_model = validate_all_models(args.model_name, args.data_version, args.mae_threshold, giskard_dataset,
                                             cfg)
            if best_model is not None:
                print(f"Best model: {best_model}")
        else:
            scan, test_results, model_version = validate_model(args.model_name, args.model_alias, args.model_version,
                                                               args.data_version, args.mae_threshold, giskard_dataset,
                                                               cfg)
